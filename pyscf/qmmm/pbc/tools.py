...
def get_multipole_tensors_pg(Rij, eta, multipole_orders, rij=None):
    ...
    if isinstance(eta, float):
        ...
        if max(multipole_orders) > 1:
            ...
            Tijab -= np.ones_like(rij)[:,:,None,None] * np.eye(3)[None,None,:,:]
            ...
            Tijab += 4/3*eta**3/np.sqrt(np.pi)* ekR[:,:,None,None] * np.eye(3)[None,None,:,:]
            res_dict[2] = Tijab
        if max(multipole_orders) > 2:
            ...
            ekRR = ekR[:,:,None] * Rij
            Tijabc -= 8/5 / np.sqrt(np.pi) * eta**5 * ekRR[:,:,:,None,None] * np.eye(3)[None,None,None,:,:]
            Tijabc -= 8/5 / np.sqrt(np.pi) * eta**5 * ekRR[:,:,None,:,None] * np.eye(3)[None,None,:,None,:]
            Tijabc -= 8/5 / np.sqrt(np.pi) * eta**5 * ekRR[:,:,None,None,:] * np.eye(3)[None,None,:,:,None]
            res_dict[3] = Tijabc
    else:
        ...
        if max(multipole_orders) > 1:
            ...
            Tijab = Tijab * invr5[:,:,None,None]
            eekR = eta[None,:]**3 * ekR
            Tijab += 4/3/np.sqrt(np.pi)* eekR[:,:,None,None] * np.eye(3)[None,None,:,:]
            res_dict[2] = Tijab
        if max(multipole_orders) > 2:
            invr7 = invr5 / rij**2 + 8/15 / np.sqrt(np.pi) * eta**5 * ekR  # NOTE this is invr7 * r**2
            Tijabc  = -15 * Rij[:,:,:,None,None] * Rij[:,:,None,:,None] * Rij[:,:,None,None,:]\
            ...
            Tijabc -= 8/5 / np.sqrt(np.pi) * eekRR[:,:,:,None,None] * np.eye(3)[None,None,None,:,:]
            Tijabc -= 8/5 / np.sqrt(np.pi) * eekRR[:,:,None,:,None] * np.eye(3)[None,None,:,None,:]
            Tijabc -= 8/5 / np.sqrt(np.pi) * eekRR[:,:,None,None,:] * np.eye(3)[None,None,:,:,None]
            res_dict[3] = Tijabc
...
def get_qm_octupoles(mol, dm):
    ...
    for i in range(mol.natm):
        ...
        p0, p1 = aoslices[i, 2:]
        qm_octupoles.append(
            -np.einsum('uv,xyzvu->xyz', dm[p0:p1], np.asarray(s1rrr)))
...
def energy_octupole(coords1, coords2, octupoles, charges):
    ...
    for i0, i1 in lib.prange(0, len(coords1), blksize):
        ...
        Rij = np.where(np.abs(Rij) < 1e-14, 1e100, Rij)
        Tijabc = get_multipole_tensors_pp(Rij, [3])[0]
        vj = np.einsum('ijabc,iabc->j', Tijabc, octupoles[i0:i1])
...
def energy_octupole_2(coords1, coords2, dipoles, quadrupoles):
    ...
    for i0, i1 in lib.prange(0, len(coords1), blksize):
        ...
        Rij = np.where(np.abs(Rij) < 1e-14, 1e100, Rij)
        Tijabc = get_multipole_tensors_pp(Rij, [3])[0]
        vj = np.einsum('ijabc,ia,jbc->', Tijabc, dipoles[i0:i1], quadrupoles)
        ene += vj / 6.0
    return float(ene)
...
def estimate_error(mol, mm_coords, a, mm_charges, rcut_hcore, dm, precision=1e-8, unit='angstrom', mf=None):
    ...
    while True:
        ...
        for shift in loop_icell(icell, a):
            ...
            dist2 = np.einsum('ix,ix->i', dist2, dist2)
